...
run_limma_and_lfc <- function(
  ...
  calc_lfc_for <- stringr::str_replace(dimnames(contrast_matrix)$Contrast, "-", "_vs_")
...
calc_lfc <- function(comparison, means){
  ...
  means %>%
    dplyr::mutate(
      !!paste0("lfc_", comparison) := !!dplyr::sym(columns[1]) - !!dplyr::sym(columns[2])
    )
}
